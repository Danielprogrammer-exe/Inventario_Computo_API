...
class Pic
{
    ...
    public function db($table, $column, $id)
    {
        ...
        if ($pic = $this->go()) {
            if (DB::table($table)->where('id', $id)->update([$column => $pic . '?t=' . time()])) {
                return $pic;
            } else {
                return null;
            }
    ...
    public static function url($pic, $thumb = false, $placeholder = null)
    {
        if (empty($pic)) {
            if ($placeholder)
                return env('URL_API')  . $placeholder;
            else
                return '';
        } else {
    ...
    public static function save($file_key, $identifier = null, $prefix = null)
    {
        ...
        if (isset($file['tmp_name']) && !empty($file['tmp_name'])) {
            $pic = $prefix . '_' . $identifier . '.' . pathinfo($file['name'], PATHINFO_EXTENSION);
