...
class Installer
{
    ...
    {
        if (!isset($this->ssh)) {
            $this->ssh = new SSH2(env('SSH_IP'));
            $this->ssh->login(env('SSH_USER'), env('SSH_PASS'));
        }
        return $this->ssh;
    }
    ...
    public function crons()
    {
        return [
            ...
            new Cron('0', '13', '*', '*', '*', 'curl ' . env('URL_API'). '/crons/daily'),
            # todos los dias a las 18h (-4)
            new Cron('0', '22', '*', '*', '*', 'curl ' . env('URL_API') . '/crons/daily'),
        ];
    }
...
MYSQL_USR="' . env('DB_USERNAME') . '"
MYSQL_PWD="' . env('DB_PASSWORD') . '"
MYSQL_HST="' . env('DB_HOST') . '"
MYSQL="$(which mysql)"
...
for db in $DBS; do
  ...
  $MYSQLDUMP --single-transaction -u $MYSQL_USR -h $MYSQL_HST -p$MYSQL_PWD $db | $GZIP -9 >$FILE
    ...
    {
        $fildir = new Fildir(Fildir::TYPE_DIR);
        $fildir->name = env('DB_DATABASE');
        $fildir->path = $this->pathMysqlBackups() . '/' . env('DB_DATABASE');
        $fildir->size = 0;
        $fildir->items = [];

        $path = $this->pathMysqlBackups() . '/' . env('DB_DATABASE');
